...
- **init-sysctl no longer CrashLoopBackOffs on read-only /proc/sys (prod outage).**
  The `init-sysctl` init container (which sets `net.ipv4.ip_forward=1`) was
  dropped to `privileged: false` + `CAP_NET_ADMIN` in an earlier hardening pass.
  On most CRIs (containerd on YC/GKE-class clusters) `/proc/sys` is remounted
  read-only for any non-privileged container, so `sysctl -w` failed with
  "Read-only file system" regardless of caps — the pod never got past init and
  ovpn-admin was `0/1` in production. Restored `privileged: true` on this
  ephemeral busybox init (runs one sysctl and exits before the app containers;
  same pattern as kube-proxy/CNI). The app containers stay unprivileged.
...
- **ovpn-admin crashed with `open /etc/openvpn/ccd/…: permission denied` when run
  as non-root.** The `init-dirs` init container created the CCD emptyDir with
  `chmod 700`, leaving it root-only, so the non-root ovpn-admin container (uid
  1000) couldn't write per-user CCD files. It now uses `chmod 2775`: the dir is
  group-owned by the fsGroup (ovpnshared/2000) that the container's process
  joins, setgid makes new files inherit the group, and `o+rx` lets openvpn read
  them. Only surfaced once the container actually ran non-root (2.0.40).
...
- **ovpn-admin container failed to start (`CreateContainerConfigError`) on
  clusters enforcing `runAsNonRoot`.** The image declared its user by name
  (`USER ovpnadmin`) while the chart set `runAsNonRoot: true` without a numeric
  `runAsUser`, so the kubelet couldn't verify the user is non-root and refused to
  create the container. The `ovpnadmin` user now has a pinned **uid 1000**
  (`USER 1000` in the image) and the chart sets `runAsUser: 1000`.
